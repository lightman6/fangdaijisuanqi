...
double power1(double x, int y, double z)
{
    qDebug()<<"函数已调用";
    double m = 1;
    m = pow((1+z),y);
...
double power2(double x, double y, int z)
{
    double m =0;
    for(int i = 0; i<z; i++)
    {
...
{
    if(ui->btn_2->isChecked()||ui->btn_1->isChecked())
    {
  ...
  //将界面的数据传输回来。
      ...
      x = 10000*x;
      switch (ui->comboBox_2->currentIndex())//按揭成数
    {
    case 0:
        y = 0.8;
        break;
...
//年数
  ...
  //数据传输完毕，开始计算及传输到界面。
          qDebug()<<"每月月供"<<power1(sum1, z, w);
          sum2 = power1(sum1, z, w);
    ...
    {
        QMessageBox::warning(NULL, "warning", "请选择计算方式", QMessageBox::Yes | QMessageBox::No, QMessageBox::Yes);
    }
 //   double time_End = (double)clock();
 //   qDebug()<<"程序消耗时间为："<<(time_End - time_Start)/1000.0<<"s";
    if(ui->out_4->text() == "nan")
    {
        QMessageBox::critical(NULL, "critical", "输入错误，请重试", QMessageBox::Yes | QMessageBox::No, QMessageBox::Yes);
        ui->widget_11->hide();
        ui->out_1->clear();
        ui->out_2->clear();
        ui->out_3->clear();
